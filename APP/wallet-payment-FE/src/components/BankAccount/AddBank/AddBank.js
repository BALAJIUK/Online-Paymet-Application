...
export const AddBank = () => {
  ...
  const addaccount = (e) => {
      ...
      if(formEl.current.checkValidity() && name!=="Select Your Bank"){
        dispatch(addAccount({
            bankName:name,
            accountNo:account,
            ifscCode:code,
  ...
  return (
    ...
    <div className="container">
      <div className="row mt-3">
        ...
        <div className="col-sm">
          <button className="text-center h4 notactive mt-3 p-1" onClick={banks}>
            Bank Accounts
          </button>
        </div>
        <div className="col-sm">
      ...
      { state.bank.addSuccess===true && (
            <h4 className="alert alert-success text-center" style={{borderRadius:"25px"}}>{state.bank.message}</h4>
        )}
        { state.bank.errorStatus===true && (
            <h4 className="alert alert-danger text-center" style={{borderRadius:"25px"}}>{state.bank.error}</h4>
        )}
      </div>
      <form ref={formEl} className="need-validation" noValidate>
      <label className="b mt-5">
                  Bank Name:
              </label>
        <Form.Control
